	...
	public function mk_npad(){//签写留言
		...
		return "
			<div id=\"comm\">&nbsp;&nbsp;&nbsp;<span class=\"red\">签写留言:</span><a name=\"npad\"></a>
				<form action=\"./?do=1\" method=\"post\" name=\"npad_form\" id=\"npad_form\" onsubmit=\"return isub();\">
					<p class=\"mid\"><textarea style=\"width:80%;height:80px;padding:6px;border:1px solid #aaa;box-shadow:3px 3px 3px #aaa;\" onpropertychange=\"if(value.length>500)value=value.substr(0,500)\" name=\"words\" id=\"words\" placeholder=\"欢迎留言: 30秒间隔限制\" required=\"required\" ></textarea></p>
					<p class=\"right\"><label for=\"guest\">姓名: </label><input name=\"guest\" id=\"guest\" size=\"18\" value=\"\" maxlength=\"8\" placeholder=\"匿名\" required=\"required\"  />&nbsp;&nbsp;
					<label for=\"vnum\">验证码: </label>&nbsp;<input id=\"vnum\" name=\"val\" size=\"4\" value =\"\" required=\"true\"  onfocus=\"javascript:document.getElementById('val').innerHTML='".$val_str."';\"  />&nbsp;<span id=\"val\"></span>&nbsp;<input type=\"hidden\" id=\"gid\" name=\"gid\" value=\"$gid\" /><input type=\"submit\" value=\" 发布 \" name=\"btn\" id=\"btn\" /> &nbsp;</p>
				</form>
			</div>
...
function isub(){
	...
	public function reply_npad(){//回复留言
		if($this->log_in != ""){
			...
			<table style=\"margin:auto;width:95%;padding:10px;\"><tr><td>
				<form name=\"iform\" action=\"./?pg=$pg&do=2&act=save&id=$id\" method=\"post\" onsubmit='return formCheck()'>
					<p class=\"mid\"><textarea style=\"margin:auto;padding:6px;width:90%;height:100px;\" name=\"reply\" id=\"reply\">$r</textarea></p>
					<p class=\"fright\"> <span style=\"font-size:10px;color:#888;\">[ 支持HTML回复 ]</span> <input type=\"submit\" value=\" 提交回复 \"> &nbsp;<a href=\"./\">放弃回复</a>&nbsp;</p>
				</form>
			</td></tr></table>\n";
			}
		}else{
	...
	public function show_npad(){//显示留言
		...
		for($i=$pg*$pnum;$i<$pg*$pnum+$pnum;$i++){
			if($i<$tt){
				$idx = $tt - $i;
				$lnk = $this->log_in != "" ? "<span class=\"fright\"><a href=\"./?do=3&id=".($idx-1).$back."\">删除</a>&nbsp;<a href=\"./?do=2&id=".($idx-1).$back."\">回复</a>" : "";
