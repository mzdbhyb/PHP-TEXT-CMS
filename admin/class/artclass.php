...
class art extends web{
	...
	function __construct(){
		...
		$this->art_file = $this->sd."/".$this->art.".php";
	...
	function mk_cont(){//文章内容
		if(is_file($this->art_file)){
			...
			echo "\t\t\t<div class=\"position\">位置: <a href=\"../\">首页</a> > <a href=\"./?id=".$this->idx."\">".$this->sort_ar[$this->idx][0]."</a><span class=\"fright\">[ 发布时间: ".$this->create_tm." &nbsp;作者: ".$this->author.$this->counter()." ]</span></div>\n";
			echo "\t\t\t<div class=\"cont\" style=\"padding:10px 30px;font-size:12pt;\">\n\n".$this->contents.$this->art_nav()."\n\t\t\t</div>\n";
			$this->comm();
			$bs = strstr($_SERVER['PHP_SELF'],"bshow.php")=="bshow.php" ? "b" : "m";
			if(isset($_GET['do']) && $_GET['do'] == "save") $this->sav_comm($bs);//处理提交的评论
		}else{
			$search = "../art/search.php?idx=20&keyword=$this->art";
			echo "\n\t\t\t<blockquote>所请求的文章路径出错！整站系统将尝试在站内查找。请稍候 ...<br /><br /> 或者请点击 >> <a href=\"$search\">站内搜索</a></blockquote>\n<script language=\"javascript\">\nvar timerID=setTimeout(\"window.location='".$search."'\",2000);\n</script>\n";
		}
	}

	public function counter(){//阅读计数
		$numfile = $this->sd."c/".$this->art."_c.txt";
			...
			for($i=0;$i<$num;$i++) {
				...
				if($this->art == $a) {
					if($i==0) {
						...
						$a_file = $pb == "b" ? "bshow.php" : "show.php";
						$psd = trim($psd);
						$str .= "前一篇: <a href=\"$a_file?st=$this->st&sd=$psd&art=$pa\">".stripslashes($pt)."</a><br />";
					}
					if($i==$num-1) {
						$str .= "下一篇: 没有了<br />";
					}else{
						@list($ns,$na,$nt,$nau,$nb,$nct,$nsd) = explode("#",$ar[$i+1]);
						$a_file = $nb == "b" ? "bshow.php" : "show.php";
						$nsd = trim($nsd);
						$str .= "下一篇: <a href=\"$a_file?st=$this->st&sd=$nsd&art=$na\">".stripslashes($nt)."</a><br />";
					}
					break;
				}
			}
		...
		if($this->art_pl == 1 && $do != 'save'){
			...
			<div id=\"comm\">发表评论:<br />
				<form action=\"".$_SERVER['PHP_SELF']."?st=$this->st&sd=$this->sd&art=$this->art&do=save\" method=\"post\" name=\"comm_form\" id=\"comm_form\" onsubmit=\"return isub();\">
					<p style=\"text-align:center;\"><textarea style=\"width:80%;height:80px;padding:6px;border:1px solid #aaa;box-shadow:3px 3px 3px #aaa;\" onpropertychange=\"if(value.length>500)value=value.substr(0,500)\" name=\"comm_words\" id=\"comm_words\" required=\"required\" placeholder=\"欢迎评论: 30秒间隔限制\"></textarea></p>
					<p style=\"text-align:right;\"><label for=\"guest\">姓名: </label><input name=\"guest\" id=\"guest\" size=\"18\" value=\"\" maxlength=\"8\" placeholder=\"name\" required=\"required\" />&nbsp;&nbsp;
					...
					<input type=\"submit\" value=\" 发布 \" id=\"okey\" name=\"okey\" /> &nbsp;</p>
				</form>\n";
			$comm_db_file = $this->sd ."c/".$this->art.".txt";//显示评论
			...
			if($comm_tt > 0){
				echo "\t\t\t\t<blockquote class=\"shadow\" style=\"background:#efefef;color:#333;padding:8px 6px;font-size:12px;\">\n";
				for($i=0;$i<$comm_tt;$i++) {
					if($i<$limit) {
						if(isset($comm_ar)) {
