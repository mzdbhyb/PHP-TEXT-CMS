...
session_start();
$ulogin =  @$_SESSION['SYS_admin'];
if($ulogin == "") die('user wrong');

$inputName='filedata';//表单文件域name
$attachDir='../upload';//上传文件保存路径
$maxAttachSize=1048576;//最大上传大小1Mb
...
if($err==''){
	...
	if(preg_match('/'.str_replace(',','|',$upExt).'/i',$extension))
	{
		...
		{
			$attachDir = $attachDir.'/'.date('Y');//按年存储
			if(!is_dir($attachDir))
			{
			...
			$newFilename =($ulogin != 'master' ? $ulogin."_" : "").date("YmdHis").mt_rand(1000,9999).'.'.$extension;
			$targetPath = $attachDir.'/'.$newFilename;
			
			rename($tempPath,$targetPath);

			$dbFile = "../upload/upadmin.txt";//保存记录
			$savstr = $targetPath."#".$newFilename."\n";
			$fp = fopen($dbFile,'a');
			flock($fp,LOCK_EX);
			fwrite($fp,$savstr);
			flock($fp,LOCK_UN);
